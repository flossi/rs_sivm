#!/home/hermanns/.conda/envs/flowers/bin python3
# -*- coding: utf-8 -*-
...
def HSI2RGB(wY, HSI, ydim, xdim, d, threshold):
    '''
    Args:
        wY (list): band specifications of hyperspectral dataset in [nm]
        HSI (numpy.ndarray): A  transformed hyperspectral image cube.
            Dimensions must be defined as (pixels, bands)
        ydim / xdim (int): Extent of HSI in [px]
        d {50, 55, 65, 75}: Determines the illuminant used (use 65 if in doubt)
        threshold (float): Choose a clipping value to increase contrast
    ...
    International Geoscience and Remote Sensing Symposium, Virtual Symposium, 2020
    ...
    D = spio.loadmat('/home/hermanns/Work/Python/fmch/fmch/D_illuminants.mat')
    w = D['wxyz'][:,0]
    x = D['wxyz'][:,1]
    y = D['wxyz'][:,2]
    z = D['wxyz'][:,3]
